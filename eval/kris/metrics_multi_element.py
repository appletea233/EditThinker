...
from kris.metrics_common import extract_score_and_reason
from kris.prompts import (
...
def compress_image_if_needed(image_path, max_size_mb=3.0):
    """Compress image if its base64 size exceeds max_size_mb."""
    try:
        ...
        for quality in [85, 75, 65, 55, 45]:
            ...
            size_mb = len(b64_str) / (1024 * 1024)
            
            if size_mb <= max_size_mb:
                if quality < 85:
                    logging.debug(f"Compressed image {image_path} to {size_mb:.2f}MB (quality={quality})")
                return b64_str
        ...
        while scale_factor > 0.3:
            ...
            buffer = io.BytesIO()
            resized_img.save(buffer, format='JPEG', quality=65, optimize=True)
            buffer.seek(0)
            b64_str = base64.b64encode(buffer.getvalue()).decode("utf-8")
            size_mb = len(b64_str) / (1024 * 1024)
            
            if size_mb <= max_size_mb:
                logging.info(f"Resized and compressed image {image_path} from {original_size} to {new_size}, size: {size_mb:.2f}MB")
                return b64_str
            
            scale_factor -= 0.1
        
        logging.warning(f"Image {image_path} still large ({size_mb:.2f}MB) after compression, using smallest version")
        return b64_str
...
def encode_image_to_base64(image_path):
    """Encode an image file to a base64 string, with automatic compression if needed."""
    try:
        # First try to read and encode directly
        with open(image_path, "rb") as f:
            ...
            logging.info(f"Image {image_path} is {size_mb:.2f}MB, compressing...")
            return compress_image_if_needed(image_path, max_size_mb=3.0)
...
def evaluate_multi_element_images(model_name, category, image_id, metrics=None):
    ...
    pred_path = os.path.join(RESULTS_DIR, category, f"{image_id}.png")
...
def run_multi_element_evaluation(models=None, categories=None, metrics=None, max_workers=8):
    ...
    for model in models:
        for category in tqdm(categories, desc=f"Evaluating {model}"):
            ...
            out_dir = os.path.join(RESULTS_DIR, category)
