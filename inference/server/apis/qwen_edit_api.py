...
class EditGPUWorker:
    ...
    def initialize_model(self):
        ...
        try:
            ...
            self.pipe = QwenImageEditPipeline.from_pretrained(self.model_repo_id, torch_dtype=torch_dtype)
            self.pipe = self.pipe.to(self.device)
            self.pipe.set_progress_bar_config(disable=None)
            print(f"GPU {self.gpu_id} 图像编辑模型初始化成功")
            return True
        except Exception as e:
            print(f"GPU {self.gpu_id} 图像编辑模型初始化失败: {e}")
            return False
    ...
    def run(self):
        ...
        print(f"GPU {self.gpu_id} 编辑worker 启动")
        ...
        while not self.stop_event.is_set():
            try:
                ...
                print(f"GPU {self.gpu_id} 编辑worker 异常: {e}")
                continue
        
        print(f"GPU {self.gpu_id} 编辑worker 停止")
...
class MultiGPUEditManager:
    def __init__(self, model_repo_id, num_gpus=None, task_queue_size=100):
        ...
        print(f"初始化多GPU图像编辑管理器，使用 {self.num_gpus} 个GPU，队列大小 {task_queue_size}")
    ...
    def start_workers(self):
        ...
        print(f"所有 {self.num_gpus} 个GPU编辑workers 已启动")
...
async def lifespan(app: FastAPI):
    """应用生命周期管理"""
    # 启动时初始化GPU管理器
    print("正在启动图像编辑API服务器...")
    initialize_gpu_manager()
    if gpu_manager is None:
        print("警告: GPU管理器初始化失败，某些功能可能不可用")
    else:
        print("图像编辑API服务器启动完成")
    
    yield
    
    # 关闭时清理资源
    print("正在关闭图像编辑API服务器...")
    if gpu_manager:
        gpu_manager.stop()
    print("图像编辑API服务器已关闭")

app = FastAPI(
    title="Qwen-Image-Edit API服务器",
    description="基于Qwen-Image-Edit模型的图像编辑API服务",
    version="1.0.0",
    docs_url="/docs",
    redoc_url="/redoc",
    lifespan=lifespan
...
async def root():
    """根端点"""
    return {"message": "欢迎使用Qwen-Image-Edit API服务器", "status": "运行中"}
...
def edit_image(request: ImageEditRequest):
    ...
    try:
        ...
        if result['success']:
            ...
            print(f"图像编辑成功，使用GPU {result['gpu_id']}，耗时 {processing_time:.2f} 秒")
...
def edit_image_upload(
    ...
    guidance_scale: float = Form(4.0, description="引导尺度"),
