#!/usr/bin/env python3
"""
Qwen-Image Lightning API服务器
基于demo.py的GPU管理器，提供REST API接口，支持Lightning模型
"""
...
lora_path = "/path/to/Qwen-Image-Lightning-8steps-V1.1.safetensors"
...
print(f"Lightning LoRA路径: {lora_path}")


# ============== GPU管理器类（Lightning版本） ==============
class LightningGPUWorker:
    ...
    def initialize_model(self):
        ...
        try:
            ...
            print(f"GPU {self.gpu_id} Lightning模型初始化成功")
            return True
        except Exception as e:
            print(f"GPU {self.gpu_id} Lightning模型初始化失败: {e}")
            return False
    ...
    def run(self):
        ...
        print(f"GPU {self.gpu_id} Lightning worker 启动")
        ...
        while not self.stop_event.is_set():
            try:
                ...
                print(f"GPU {self.gpu_id} Lightning worker 异常: {e}")
                continue
        
        print(f"GPU {self.gpu_id} Lightning worker 停止")


# 全局GPU worker函数，用于spawn模式
def lightning_gpu_worker_process(gpu_id, model_repo_id, lora_path, task_queue, result_queue, stop_event):
    worker = LightningGPUWorker(gpu_id, model_repo_id, lora_path, task_queue, result_queue, stop_event)
    worker.run()


class MultiGPULightningManager:
    def __init__(self, model_repo_id, lora_path, num_gpus=None, task_queue_size=100):
        ...
        print(f"初始化多GPU Lightning管理器，使用 {self.num_gpus} 个GPU，队列大小 {task_queue_size}")
        
    def start_workers(self):
        """启动所有GPU workers"""
        for gpu_id in range(self.num_gpus):
            process = Process(target=lightning_gpu_worker_process, 
        ...
        print(f"所有 {self.num_gpus} 个GPU Lightning workers 已启动")
    ...
    def submit_task(self, prompt, negative_prompt="", seed=42, width=1664, height=928, 
        ...
        """提交任务并等待结果"""
        with self.pending_tasks_lock:
            task_id = f"lightning_task_{self.task_counter}_{time.time()}"
            self.task_counter += 1
...
def initialize_gpu_manager():
    ...
    if gpu_manager is None:
        try:
            ...
            gpu_manager = MultiGPULightningManager(
                model_repo_id, 
                lora_path,
            ...
            print("GPU Lightning管理器初始化成功")
        except Exception as e:
            print(f"GPU Lightning管理器初始化失败: {e}")
            gpu_manager = None
...
class LightningImageGenerationRequest(BaseModel):
    ...
    guidance_scale: Optional[float] = Field(1.0, ge=0.0, le=7.5, description="引导尺度（Lightning模型推荐1.0）")
    num_inference_steps: Optional[int] = Field(8, ge=1, le=50, description="推理步数（Lightning模型推荐8步）")
    enhance_prompt: Optional[bool] = Field(True, description="是否增强提示词")

class LightningImageGenerationResponse(BaseModel):
    ...
    model_type: str = Field("Lightning", description="模型类型")
...
app = FastAPI(
    title="Qwen-Image Lightning API服务器",
    description="基于Qwen-Image Lightning模型的快速图像生成API服务",
    version="1.0.0",
    docs_url="/docs",
    redoc_url="/redoc"
)
...
async def startup_event():
    """应用启动时初始化GPU管理器"""
    print("正在启动Lightning API服务器...")
    initialize_gpu_manager()
    if gpu_manager is None:
        print("警告: GPU Lightning管理器初始化失败，某些功能可能不可用")
    else:
        print("Lightning API服务器启动完成")

@app.on_event("shutdown")
async def shutdown_event():
    """应用关闭时清理资源"""
    print("正在关闭Lightning API服务器...")
    if gpu_manager:
        gpu_manager.stop()
    print("Lightning API服务器已关闭")

@app.get("/")
async def root():
    """根端点"""
    return {"message": "欢迎使用Qwen-Image Lightning API服务器", "status": "运行中", "model_type": "Lightning"}
...
async def get_system_status():
    """获取系统状态"""
    if gpu_manager is None:
        raise HTTPException(status_code=503, detail="GPU Lightning管理器未初始化")
...
@app.post("/generate", response_model=LightningImageGenerationResponse)
def generate_image(request: LightningImageGenerationRequest):
    """生成图像（Lightning版本）"""
    ...
    try:
        ...
        if result['success']:
            ...
            print(f"Lightning图像生成成功，使用GPU {result['gpu_id']}，耗时 {generation_time:.2f} 秒")
            
            return LightningImageGenerationResponse(
                success=True,
                task_id=result['task_id'],
                image=image_b64,
                seed=seed,
            ...
            raise HTTPException(
                status_code=500, 
                detail=f"Lightning图像生成失败: {result['error']}"
            )
            
    except HTTPException:
        raise
    except Exception as e:
        error_msg = f"生成Lightning图像时发生异常: {str(e)}"
        print(error_msg)
