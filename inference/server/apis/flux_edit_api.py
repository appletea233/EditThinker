...
model_repo_id = "/path/to/FLUX.1-Kontext-dev"  # Replace with your FLUX.1-Kontext-dev model path
...
# ============== GPU管理器类（适配FLUX Kontext图像编辑） ==============
class FluxEditGPUWorker:
    ...
    def initialize_model(self):
        ...
        try:
            ...
            print(f"GPU {self.gpu_id} FLUX Kontext图像编辑模型初始化成功")
            return True
        except Exception as e:
            print(f"GPU {self.gpu_id} FLUX Kontext图像编辑模型初始化失败: {e}")
            return False
            ...
            with torch.cuda.device(self.gpu_id):
                with torch.inference_mode():
                    output = self.pipe(
                        ...
                        max_area=original_width * original_height,  # 保持原始尺寸
                    ).images[0]
    ...
    def run(self):
        ...
        print(f"GPU {self.gpu_id} FLUX编辑worker 启动")
        ...
        while not self.stop_event.is_set():
            try:
                ...
                print(f"GPU {self.gpu_id} FLUX编辑worker 异常: {e}")
                continue
        
        print(f"GPU {self.gpu_id} FLUX编辑worker 停止")


# 全局GPU worker函数，用于spawn模式
def flux_edit_gpu_worker_process(gpu_id, model_repo_id, task_queue, result_queue, stop_event):
    worker = FluxEditGPUWorker(gpu_id, model_repo_id, task_queue, result_queue, stop_event)
    worker.run()


class MultiGPUFluxEditManager:
    def __init__(self, model_repo_id, num_gpus=None, task_queue_size=100):
        ...
        print(f"初始化多GPU FLUX图像编辑管理器，使用 {self.num_gpus} 个GPU，队列大小 {task_queue_size}")
        
    def start_workers(self):
        """启动所有GPU workers"""
        for gpu_id in range(self.num_gpus):
            process = Process(target=flux_edit_gpu_worker_process, 
        ...
        print(f"所有 {self.num_gpus} 个GPU FLUX编辑workers 已启动")
    ...
    def submit_task(self, image, prompt, seed=42, 
                   guidance_scale=2.5, num_inference_steps=28, timeout=300):
        """提交编辑任务并等待结果"""
        with self.pending_tasks_lock:
            task_id = f"flux_edit_task_{self.task_counter}_{time.time()}"
            self.task_counter += 1
...
def initialize_gpu_manager():
    ...
    if gpu_manager is None:
        try:
            ...
            gpu_manager = MultiGPUFluxEditManager(
                model_repo_id, 
                num_gpus=NUM_GPUS_TO_USE,
                task_queue_size=TASK_QUEUE_SIZE
            )
            gpu_manager.start_workers()
            print("GPU FLUX编辑管理器初始化成功")
        except Exception as e:
            print(f"GPU FLUX编辑管理器初始化失败: {e}")
            gpu_manager = None
...
class FluxImageEditRequest(BaseModel):
    ...
    guidance_scale: Optional[float] = Field(2.5, ge=0.0, le=7.5, description="引导尺度")
    num_inference_steps: Optional[int] = Field(28, ge=1, le=100, description="推理步数")
    enhance_prompt: Optional[bool] = Field(True, description="是否增强编辑提示词")

class FluxImageEditResponse(BaseModel):
...
async def lifespan(app: FastAPI):
    """应用生命周期管理"""
    # 启动时初始化GPU管理器
    print("正在启动FLUX图像编辑API服务器...")
    initialize_gpu_manager()
    if gpu_manager is None:
        print("警告: GPU管理器初始化失败，某些功能可能不可用")
    else:
        print("FLUX图像编辑API服务器启动完成")
    
    yield
    
    # 关闭时清理资源
    print("正在关闭FLUX图像编辑API服务器...")
    if gpu_manager:
        gpu_manager.stop()
    print("FLUX图像编辑API服务器已关闭")

app = FastAPI(
    title="FLUX.1-Kontext-dev 图像编辑API服务器",
    description="基于FLUX.1-Kontext-dev模型的图像编辑API服务",
    version="1.0.0",
    docs_url="/docs",
    redoc_url="/redoc",
    lifespan=lifespan
...
async def root():
    """根端点"""
    return {"message": "欢迎使用FLUX.1-Kontext-dev 图像编辑API服务器", "status": "运行中"}
...
@app.post("/edit", response_model=FluxImageEditResponse)
def edit_image(request: FluxImageEditRequest):
    """编辑图像 - 使用base64格式"""
    ...
    try:
        ...
        if result['success']:
            ...
            print(f"FLUX图像编辑成功，使用GPU {result['gpu_id']}，耗时 {processing_time:.2f} 秒")
            
            return FluxImageEditResponse(
                success=True,
                task_id=result['task_id'],
            ...
            raise HTTPException(
                status_code=500, 
                detail=f"FLUX图像编辑失败: {result['error']}"
            )
...
@app.post("/edit-upload", response_model=FluxImageEditResponse)
def edit_image_upload(
    ...
    guidance_scale: float = Form(2.5, description="引导尺度"),
    num_inference_steps: int = Form(28, description="推理步数"),
